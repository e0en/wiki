...
def load():
    plugins = [ListParser(), QuoteParser(), HeaderParser(), LaTeXParser(), CodeParser(), ImageParser(), FlashParser(), RedirectTo(), BackLinks()]
    return plugins
...
class HeaderParser(LinePlugin):
    ...
    def parse(self, lines, idx, page):
        ...
        text = '<h' + str(num+1) + ' id="sec_' + sec_anchor + '">' + sec_anchor + ' ' + parse_inline(title, acc) + '</h' + str(num+1) + '>\n'

        page.chapters.append({'section':sec_num, 'anchor_txt':sec_anchor, 'title':title})
...
class LaTeXParser(LinePlugin):
    ...
    def parse(self, lines, idx, acc):
        ...
        acc['eqns'].append({'section':section, 'num':eqn_num, 'content': parsed, 'anchor_txt': eqn_anchor})

        result = '<div class="eqn" id="' + eqn_anchor + '"> <img src="http://www.sitmo.com/gg/latex/latex2png.2.php?z=120&amp;eq=' + parsed + '" alt="latex eqn" />\n' + \
        '<span class="eqn_id">(' + str(section) + '.' + str(eqn_num) + ')</span>\n</div>\n'
        offset = i - idx + 1 
