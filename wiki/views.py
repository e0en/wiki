...
def read(request, args=''):
    ...
    a_prev = wikiModels.Article.objects.filter(name__lt=a.name).order_by('-name')
    a_next = wikiModels.Article.objects.filter(name__gt=a.name).order_by('name')
...
def recentChanges(request):
    ...
    {'history_list':hlist, 'site_uri':wikiSettings.site_uri}, 
    context_instance=RequestContext(request))
...
def pageList(request):
    ...
    article_list = []
    for a in alist:
        article_list.append({'uri':wikiMisc.escapeHTML(a.name), 'name':a.name})
    #article_list.sort()
...
def upload(request):
    ...
    {'form':'f', 'site_uri':wikiSettings.site_uri, 'file_names':file_names}, 
    context_instance=RequestContext(request))
...
def processUpload(request):
    if request.method == 'POST':
        ...
        filename = str(f)
        f_server = open(os.path.join(wikiSettings.upload_path, filename), 'wb+')
...
def processEdit(request, args):
    ...
    try:
        ...
        if a.content == content:
            s = (wikiSettings.site_uri + 'read/' + wikiMisc.escapeName(a.name)).encode('utf8')
    ...
    wikiMisc.addLog(wikiSettings.log_filename, request.META.get('REMOTE_ADDR'), 'Edit', article_name)
